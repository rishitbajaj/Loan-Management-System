...
export async function updateProfile(req: Request, res: Response): Promise<void> {
  ...
  const { user, bre } = await borrowerService.updateProfile(id, input, req.userDoc);
...
export async function uploadSalarySlip(req: Request, res: Response): Promise<void> {
  ...
  const user = await borrowerService.saveSalarySlip(id, req.file, req.userDoc);
  sendSuccess(res, { user }, 'Salary slip uploaded');
}

export async function downloadSalarySlip(req: Request, res: Response): Promise<void> {
  const { id } = requireUser(req);
  const slip = await borrowerService.getSalarySlip(id, req.userDoc);
